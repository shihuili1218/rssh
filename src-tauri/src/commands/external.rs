...
pub fn open_external_url(app: AppHandle, url: String) -> AppResult<()> {
    if !url.starts_with("http://") && !url.starts_with("https://") {
        return Err(AppError::config("external_non_https_url", serde_json::json!({ "url": url })));
    }
    app.opener()
        .open_url(&url, None::<&str>)
        .map_err(|e| AppError::other("external_open_url_failed", serde_json::json!({ "err": e.to_string() })))
}
