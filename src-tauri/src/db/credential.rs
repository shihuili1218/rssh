...
pub fn list(conn: &Connection) -> AppResult<Vec<Credential>> {
    let mut stmt = conn.prepare(
        "SELECT id, name, username, type, secret, save_to_remote FROM credentials",
    )?;
...
pub fn get(conn: &Connection, id: &str) -> AppResult<Credential> {
    conn.query_row(
        "SELECT id, name, username, type, secret, save_to_remote FROM credentials WHERE id = ?1",
        params![id],
        |row| {
            Ok(Credential {
                id: row.get(0)?,
...
pub fn insert(conn: &Connection, cred: &Credential) -> AppResult<()> {
    conn.execute(
        "INSERT INTO credentials (id, name, username, type, secret, save_to_remote) VALUES (?1, ?2, ?3, ?4, ?5, ?6)",
        params![
...
pub fn update(conn: &Connection, cred: &Credential) -> AppResult<()> {
    conn.execute(
        "UPDATE credentials SET name=?1, username=?2, type=?3, secret=?4, save_to_remote=?5 WHERE id=?6",
        params![
            cred.name, cred.username, cred.credential_type.as_str(),
            cred.secret.as_deref().unwrap_or(""), cred.save_to_remote as i32, cred.id,
        ],
    )?;
    Ok(())
}
